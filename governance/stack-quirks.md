...
**IG ranges must specify the exact grep scope they apply to.** Stating "predicted residual range 6–15" is meaningless without saying *which grep* — an `--include`-filtered sweep, the full repo, a single directory, or with/without the spec body itself. SPEC-018 AG-2 caught a case where AC-2's 6–15 range was for the wrong sweep (`--include=*.html,*.xml,*.txt` excludes the `.md` files the range was actually estimating). Two-axis discipline: (a) raw count band, (b) exact grep command the band applies to. Established in SPEC-018 retro.

**Spec-body lock tables skew full-repo grep baselines.** When a Pre-Implementation String Lock contains the migration content verbatim (old-string column), the spec body's contribution to a `grep -rn` full-repo count is non-trivial (~40–70 mentions on a Standard-tier migration; SPEC-018 contributed 67–68). Architects measuring an informational baseline at Arch Gate should either measure AFTER the lock table is committed, or programmatically exclude the spec file from informational sweeps via `--exclude="SPEC-NNN-*.md"`. Established in SPEC-018 retro.

**Cloudflare Pages auto-deploy is faster than the skill's "~1–2 min" framing.** SPEC-018 Deploy Gate ran the first verification curl immediately after merge confirmation and the apex already served the new canonicals. Treat the "wait 1–2 min" rule as a safety upper bound, not an actual delay. Single-file static-site changes deploy in well under a minute. Established in SPEC-018 Deploy Gate.

**Writing the gate-`Approved` line is a separate act from earning approval — keep them in separate tool calls.** The agent wrote a gate-`Approved:` decision line into the spec *before* the operator's approval at SPEC-020 Arch Gate, SPEC-021 Arch Gate, and SPEC-021 QA Gate — three times across two specs, each caught and reverted. The trigger is consistent: resolving a gate's open question or finishing QA verification creates "the work is done" momentum that reaches for the approval line in the *same* edit that records the findings. Control: never write the `**<Gate> Decision:** Approved` string in the same edit that records resolutions/IG/QA results. Record findings as `_Pending operator approval._`, surface the gate package, then — only after the operator's explicit approval token appears in the transcript — write the Approved line in its own follow-up edit. Established in SPEC-021 retro (pattern spanning SPEC-020–021).
